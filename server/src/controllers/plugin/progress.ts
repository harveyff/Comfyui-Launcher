...
export class TaskProgressManager {
  ...
  // 创建新任务
  createTask(taskId: string, pluginId: string, type: 'install' | 'uninstall' | 'disable' | 'enable', githubProxy?: string): void {
    ...
    console.log(`[进度管理] 创建任务: ${taskId}, 插件: ${pluginId}, 类型: ${type}`);
  }
  ...
  updateProgress(taskId: string, progress: number, message?: string): void {
    if (this.taskProgressMap[taskId]) {
      ...
      console.log(`[进度管理] 任务 ${taskId} 进度更新: ${progress}%`);
    }
  }
  ...
  completeTask(taskId: string, success: boolean = true, message?: string): void {
    if (this.taskProgressMap[taskId]) {
      ...
      console.log(`[进度管理] 任务 ${taskId} 完成: ${success ? '成功' : '失败'}`);
    }
  }
  ...
  cleanupCompletedTasks(): void {
    ...
    if (cleanedCount > 0) {
      console.log(`[进度管理] 清理了 ${cleanedCount} 个已完成的任务`);
    }
  }
