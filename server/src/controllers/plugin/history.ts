...
export class PluginHistoryManager {
  ...
  private async loadHistory(): Promise<void> {
    try {
      if (fs.existsSync(HISTORY_FILE_PATH)) {
        ...
        console.log(`[历史] 已加载 ${this.operationHistory.length} 条操作历史记录`);
      } else {
  ...
  private saveHistory(): void {
    try {
      ...
      console.log(`[历史] 已保存 ${this.operationHistory.length} 条操作历史记录`);
    } catch (error) {
  ...
  public async getPluginHistory(ctx: Context): Promise<void> {
    try {
      ...
      console.log(`[国际化调试] 历史记录数量: ${this.operationHistory.length}`);
      
      // 本地化历史记录前添加示例日志
      if (this.operationHistory.length > 0) {
        console.log(`[国际化调试] 第一条历史记录的原始类型: ${this.operationHistory[0].type}`);
        console.log(`[国际化调试] 类型翻译key: ${this.translateOperationType(this.operationHistory[0].type, locale)}`);
      }
